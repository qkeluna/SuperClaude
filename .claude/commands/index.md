...
**Dev**: `/test` `/troubleshoot` `/improve` `/explore-plan-code-test`
**Utils**: `/design` `/document` `/estimate` `/dev-setup` `/git` `/spawn`

## Commands w/ Primary Flags

**Dev**: `/build` --init|feature|react | `/dev-setup` --install|ci | `/test` --coverage|e2e | `/explore-plan-code-test` --skip-explore|--ultrathink
...
**EPCT**: `/explore-plan-code-test` → Complete systematic workflow
